...
print "<div class='ic-wrapper'>";
	print '<h2 class="ic-title">'. l($node->title, 'node/' . $node->nid) .'</h2>';
...
(function ($) {
	$(document).ready(function(){
	  <?php foreach ($output['images'] as $key => $image) {
		  print 'Drupal.settings.ic_images[' . $key . ']=' . drupal_json_encode($image) . ';'."\n"; 
	  }?>
	});
}(jQuery));
</script>